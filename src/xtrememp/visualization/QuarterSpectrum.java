...
/**
 * Shows a magnitude vs. frequency spectrum of the audio waveform.  This
 * visualization displays only the lowest quarter of the spectral bins computed
 * by the FFT. This allows high-resolution viewing of the frequency bins from
 * 0 Hz to 5,512.5 Hz - where most of the musically-interesting audio energy is.
 *
 * @author Keith Bromley
 */
public final class QuarterSpectrum extends AbstractSpectrumAnalyzer {

    public static final String NAME = "Quarter Spectrum";

    public static final float DEFAULT_SPECTRUM_GAIN  = 5.00F;
    public static final float DEFAULT_SPECTRUM_SLOPE = 0.00F;
    ...
    public synchronized void render(DssContext dssContext, Graphics2D g2d, int width, int height) {
        ...
        {   x = (int) (bin * binWidth);
            if(maxOverLine == true) {y = height - (int) ( height * displayedBinValues[bin] );}
            else{ y = height - (int) ( (gain + slope*bin) * displayedBinValues[bin]); }
            if(x < 0)       {x = 0;}
            if(x > width)   {x = width;}
            if(y < 0)       {y = 0;}
        ...
        for (int bin = 0; bin < binCount; bin++)
        {   x = (int) (bin * binWidth);
            y = height - (int) (gain * meanValues[bin]);
