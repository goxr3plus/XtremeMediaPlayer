...
/**
 * Shows a magnitude vs. frequency spectrum of the audio waveform
 *
 * @author Besmir Beqiri, Keith Bromley
 */
public final class FullSpectrum extends AbstractSpectrumAnalyzer
{   public static final String NAME = "Full Spectrum";
    ...
    {   g2d.setColor(backgroundColor);
        ...
        if(maxOverLine == true) {binValues = applyMaxOverLineNormalization(binValues);}
        ...
        {   x = (int) (bin * binWidth);
            if(maxOverLine == true) {y = height - (int) ( height * binValues[bin] );}
            else{ y = height - (int) ( (gain + slope*bin) * binValues[bin] ); }
            if(x < 0)       {x = 0;}
            if(x > width)   {x = width;}
            if(y < 0)       {y = 0;}
        ...
        for (int bin = 0; bin < binCount; bin++)
        {   x = (int) (bin * binWidth);
            if(maxOverLine == true) {y = height - (int) ( height * meanValues[bin] );}
            else{ y = height - (int) ( (gain + slope*bin) * meanValues[bin] );}
